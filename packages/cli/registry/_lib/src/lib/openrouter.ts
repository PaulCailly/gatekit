...
export function resolveModels(commentBody: string): ModelSpec[] {
  // Scope to the first line only: auto-appended footers (e.g. "Generated by
  // [Claude Code](https://claude.ai/code)") on later lines contain "/" tokens
  // that would otherwise be mis-parsed as raw OpenRouter slugs, causing a 400.
  const firstLine = commentBody.split(/\r?\n/)[0];
  const after = firstLine.replace(/^\/review\b/i, "").trim();
